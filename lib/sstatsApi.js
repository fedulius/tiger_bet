...
async function findGameByTeams(homeTeamName, awayTeamName, date) {
  ...
  // Last resort: search by LeagueId=1 (WC) without date filter
  const wcList = await apiGet('/Games/list', {
    LeagueId: '1',
    Year: '2026',
    limit: '200',
    TimeZone: '3',
...
async function buildMatchPayload(gameId) {
  ...
  return {
    ...
    league_id: leagueId,
    league_slug: game.season?.league?.url || game.season?.league?.slug || null,
