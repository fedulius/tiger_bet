...
function extractEditorialForecast(html, { matchName = '' } = {}) {
  ...
  const headlineMatch = plainText.match(/Прогноз на матч[\s\S]{0,250}?:\s*([\s\S]{5,260}?)(?:Основной прогноз:|Прогноз на тотал:|Поставь на исход|$)/i);
  const mainBlockMatch = plainText.match(/Основной прогноз:\s*([\s\S]{10,900}?)(?:Прогноз на тотал:|Поставь на исход|Ставка на|$)/i);

  const headlineText = headlineMatch ? headlineMatch[1].trim() : '';
  const mainBlockText = mainBlockMatch ? mainBlockMatch[1].trim() : '';

  if (!headlineText && !mainBlockText) {
    return null;
  }

  const sentences = mainBlockText
    .split(/(?<=[.!?])\s+/)
    .map((s) => s.trim())
    .filter(Boolean);

  const firstMainSentence = sentences[0] || '';
  const mainThoughtRaw = headlineText || firstMainSentence || mainBlockText;
  const mainThought = mainThoughtRaw
    .replace(/\s+/g, ' ')
    .replace(/\.+$/g, '')
    .trim();

  const coeffSource = `${headlineText} ${mainBlockText}`;
  const coeff = toNumber(coeffSource.match(/коэффициент\s*(\d{1,2}(?:[.,]\d+)?)/i)?.[1]);
  ...
  return {
    ...
    explanationLines: [
      ...
      `3) Доп. обоснование: ${firstMainSentence ? firstMainSentence.slice(0, 140) : 'в блоке нет отдельного предложения с обоснованием.'}`,
