...
test('GET /favorites returns DB-backed favorites from selected tournaments', async () => {
...
test('PUT /favorites replaces selected leagues in user_tournament only', async () => {
  ...
  try {
    ...
    assert.ok(!fakePg.calls.some((call) => /public\.user_sport/i.test(call.query)));
...
test('GET /favorites ignores user_sport when user has no selected tournaments', async () => {
  const fakePg = createFakePg({
    handler(query) {
      if (/FROM public\.user_tournament ut/i.test(query)) return [];
      if (/FROM public\.sport/i.test(query)) return [{ sport_id: 1, sport_name: 'Футбол', sport_url: 'soccer' }];
      return [];
    },
  });
  const app = buildTestApp(buildApp, { pg: fakePg });
...
test('DELETE /favorites clears user_tournament only', async () => {
  const fakePg = createFakePg({ rows: [] });
  const app = buildTestApp(buildApp, { pg: fakePg, recommendationsRedis: makeFakeRedis() });
  await app.ready();
  try {
    const response = await app.inject({
      headers: makeAuthHeaders(app, { userId: 55, telegram_user_id: 777, profile: 'telegram:777' }),
